package com.example.quiz.quiz;

import android.app.AlertDialog;
import android.content.Context;
import android.content.DialogInterface;
import android.content.Intent;
import android.os.AsyncTask;
import com.example.quiz.Quiz;
import com.example.quiz.data.Endpoint;
import com.example.quiz.utilities.ViewDialog;

import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;

/**
 * The GetQuiz is an Async class that is made for sending a HTTP Request to the endpoint
 * made for the sending random questions.
 */
public class GetQuiz extends AsyncTask<String, Void, String> {

    private String result, token;
    private ViewDialog viewDialog;
    private Context context;
    private AlertDialog.Builder myAlert;
    private int code;

    public GetQuiz(ViewDialog viewDialog, AlertDialog.Builder myAlert, Context context, String token){
        this.viewDialog = viewDialog;
        this.myAlert = myAlert;
        this.context = context;
        this.token = token;
    }

    /**
     * Start Quiz Activity
     *
     * @result      pass json that is a result of Http call
     * @token       pass token that were generated by server after successful login
     */
    public void StartGame(String result, String token) {

        Intent intent = new Intent(context, Quiz.class);
        intent.putExtra("json", result);
        intent.putExtra("token", token);
        context.startActivity(intent);
    }

    /**
     * Display wait indicator until Http request is not finished
     */
    @Override
    protected void onPreExecute() {
        super.onPreExecute();
        viewDialog.showDialog();

    }

    /**
     * Send Http request to the endpoint whose task is to generate a random question.
     *
     * @return      return result of the Http request
     */
    @Override
    protected String doInBackground(String... params) {

        try {

            //1. Create okHttp Client object
            OkHttpClient client = new OkHttpClient();
            Endpoint endpoint = new Endpoint();

            //2. Define request being sent to the server
            Request request = new Request.Builder()
                    .url(endpoint.getUrl() + "questions/random")
                    .header("QuizIdentity", token)
                    .build();

            //3. Transport the request and wait for response to process next
            try {
                Response response = client.newCall(request).execute();
                code = response.code();
                result = response.body().string();
            }catch (NullPointerException e){
                e.printStackTrace();
            }

            return result;

        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    }

    /**
     * Display error Alert
     */
    private void showFailedAlert(AlertDialog.Builder myAlert){

        myAlert.setMessage("Unexpected error!")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                    }
                })
                .create();
        myAlert.show();
    }

    /**
     * Check response code and if the response is successful then start Quiz Activity
     * else display error Alert.
     */
    @Override
    protected void onPostExecute(String s) {
        super.onPostExecute(s);
        //Check the response code, if it was successful start a game
        if (code == 200){
            viewDialog.hideDialog();
            new GetQuestions(result);
            StartGame(result, token);
        }else{
            showFailedAlert(myAlert);
        }

    }
}
